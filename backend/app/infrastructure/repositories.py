...
class UserRepository:
    ...
    async def find_all(self) -> List[User]:
        ...
        return [
            User(id=r.id, email=r.email, name=r.name, created_at=r.created_at)
            for r in rows
        ]
...
class OrderRepository:
    ...
    async def find_by_id(self, order_id: uuid.UUID) -> Optional[Order]:
        ...
        status_history: list[OrderStatusChange] = await self._load_status_history(
            order_id
        )
    ...
    async def _save_status_history(
        self, status_history: list[OrderStatusChange]
    ) -> None:
        query: TextClause = text(
            """
